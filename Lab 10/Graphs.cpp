...
struct Graph{
    ...
    void intializenodes(){
        ...
        for(int i=0;i<n;i++){
            nodes[i].label=i+1;     // Add labels starting from 1
        }
    }

    void addedge(int u, int v){
        //select node u and push v into u's neighbour
        nodes[u-1].neighbours.push_back(v);             // We indexed the array 'nodes' as start from 0

        //select node v and push u into v's neighbour
        nodes[v-1].neighbours.push_back(u);             // We indexed the array 'nodes' as start from 0
    }
